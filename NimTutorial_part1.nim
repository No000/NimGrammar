...
for i in countup(1, 11):    # 変数iは暗黙的に宣言されている
  echo i
# countup関数は組み込み関数で、指定の範囲を数え上げて行く。
# 類似コードとして
echo "Counting to 10 :"
var i = 1
while i <= 10:
...
# String型
#[
  Nim言語においての文字列の数は可変なので、後から内容の追加を行うことができます。
  Nimの文字列は、両方の終端に0をもち、文字列フィールドをもつ。
  文字列の長さを取得するには、lenプロシージャを使う（この場合終端の0は含めない）
  終端に存在する0にｱｸｾｽするとエラーとなる、なぜならNim文字列をcstringに変換でき
  るようにするために存在しているからです。

  文字列の代入演算子は文字列をコピーする。
  文字列を連結するには、演算子&を使用し
  文字列に追加をする場合はaddプロシージャを使用する。

  文字列は、辞書式順序を利用して比較が行われます。また、全ての比較演算子がサポートされています。
  全ての文字列はUTF-8 でデコードされていますが、これは強制はされていません。
  例えば、バイナリファイルから文字列を読み取る場合、それらは単なるバイトシーケンスです。
  よって、インデックス演算子 s[i]はi番目のunicharを示しているのではなく、sにおけるi番目のcharを示しています。

  文字列変数を初期化するに" "で初期化をする。
]#

#----------------------------------------------------------------------------------------------------------------------------
# 整数型
#[
  Nimには、int, int8, int16, int32, int64, uint, uint8, uint16, uint32, uint64 が組み込まれている。
  デフォルトの整数型はintとして扱われます。デフォルト以外の整数型を指定したい場合は、それを指定するた
  めの接尾辞を付けることができる。
]#
...
#[
  ほとんどの場合、整数型はメモリーの中でオブジェクトを数えるために使用されるため、intはポインター(アドレス)と同じサイズ。（GCとかの処理系依存？）
  整数型では、よく使われる演算子である+, -, *, div, mod, <, <=, ==, !=, !=, >, >=を使用することができる。
  ビット演算子である、and, or, xor, notも使用も定義されている。
]#